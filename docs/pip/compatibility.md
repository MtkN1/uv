...
uv は `pip` よりも厳格であり、`pip` がインストールするパッケージを拒否することがよくあります。たとえば、uv は無効な URL フラグメントを含む HTML インデックスを拒否します（参照：[PEP 503](https://peps.python.org/pep-0503/)）、一方 `pip` はそのようなフラグメントを無視します。
